...
public class CommandExecutorTests {
    ...
    public void ConnectionString_構造參數是否覆蓋Configuration設定_應該設置為指定的值() {
        ...
        CommandExecutor executor = new();
        Assert.That(executor.ConnectionString, Is.SameAs(ConnectionString));

        executor = new(null, null);
        Assert.That(executor.ConnectionString, Is.SameAs(ConnectionString));

        executor = new("Test");
        Assert.That(executor.ConnectionString, Is.SameAs("Test"));
    }
    ...
    public void KeepConnection_構造參數傳入bool時覆蓋預設值_應該設置為指定值() {
        ...
        CommandExecutor executor = new();
        Assert.That(executor.KeepConnection, Is.EqualTo(true));

        executor = new(null);
        Assert.That(executor.KeepConnection, Is.EqualTo(true));

        executor = new(false);
        Assert.That(executor.KeepConnection, Is.EqualTo(false));
    }
    ...
    public void CreateDataReader_應回傳符合查詢條件的記錄集合(bool keepConnection) {
        ...
        Assert.That(list, Is.EquivalentTo(BasicRecords));
    }
    ...
    public void Query_Where條件有使用且ParameterValue為IEnumerable_會換成複數Parameter() {
        ...
        Assert.That(records.ElementAt(0), Is.EqualTo(BasicRecords.Single(x => x.Id == 1)));
        Assert.That(records.ElementAt(1), Is.EqualTo(BasicRecords.Single(x => x.Id == 2)));
    }
    ...
    public void BeginTransaction_KeepConnection為False_ShouldThrowKeepConnectionRequiredException() {
        ...
        Assert.That(() => act(), Throws.TypeOf<KeepConnectionRequiredException>().And.Message.EqualTo("KeepConnection must be set to true to use BeginTransaction."));
    }
    ...
    public void Initialize(ResetItems items) {
        ...
        if ((items & ResetItems.Parameters) == ResetItems.Parameters) {
            Assert.That(executor.Parameters.Count, Is.EqualTo(0));
        }
    }
